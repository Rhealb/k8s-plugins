...
//service use annotations to specify aws loadbalancer name
// TagNameServiceLoadbalancer is the tag name we use to bind service to an loadbalancer
...
//load balancer name
const ServiceAnnotationAWSLoadBalancerName = "enndata.io/aws-load-balancer-name"

const ServiceAnnotationLoadBalancerType = "service.beta.kubernetes.io/aws-load-balancer-type"

const ServiceAnnotationAWSLoadBalancerInternal = "service.beta.kubernetes.io/aws-load-balancer-internal"

// TODO(#6812): Use a shorter name that's less likely to be longer than cloud
// providers' name length limits.
func GetLoadBalancerName(service *v1.Service) string {
	if service.Annotations == nil {
		//classic type loadbalancer
		return OriginGetLoadBalancerName(service)
	}
	if service.Annotations[ServiceAnnotationLoadBalancerType] == "" {
		// not in aws cloud, because in aws we do not use classic type lb
		return OriginGetLoadBalancerName(service)
	}

	//return service annotation loadbalancer specify name
	if service.Annotations[ServiceAnnotationAWSLoadBalancerName] != "" {
		return service.Annotations[ServiceAnnotationAWSLoadBalancerName]
	}

	internalAnnotation := service.Annotations[ServiceAnnotationAWSLoadBalancerInternal]
...
func OriginGetLoadBalancerName(service *v1.Service) string {
